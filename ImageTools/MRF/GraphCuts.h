...
/*
    For description, example usage, discussion of GraphCuts representation
...
namespace mrf
{
	...
	public:
	    ...
	    GraphCuts(void (*err_function)(const char *) = NULL);
	
	    /* Destructor */
	    ~GraphCuts();
	
	    /* Adds a node to the GraphCuts */
	    node_id add_node();
	    ...
	    /* After the maxflow is computed, this function returns to which
	       segment the node 'i' belongs (GraphCuts::SOURCE or GraphCuts::SINK) */
	...
	#define last_node LAST_NODE.LAST_NODE
	    ...
	    typedef struct arc_for_block_st
	    {
	        ...
	        {
	            arc_forward         dummy;
	            node                *LAST_NODE; /* used in GraphCuts consruction */
	        }                       LAST_NODE;
	    } arc_for_block;
