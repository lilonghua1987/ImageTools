...
namespace mrf
{
	GraphCuts::GraphCuts(void (*err_function)(const char *))
	{
	...
	GraphCuts::node_id GraphCuts::add_node()
	{
	    node *i;
	...
	void GraphCuts::add_edge(node_id from, node_id to, captype cap, captype rev_cap)
	{
	    arc_forward *a_for;
	    arc_reverse *a_rev;
	...
	void GraphCuts::set_tweights(node_id i, captype cap_source, captype cap_sink)
	{
	    flow += (cap_source < cap_sink) ? cap_source : cap_sink;
	    ((node*)i) -> tr_cap = cap_source - cap_sink;
	}
	
	void GraphCuts::add_tweights(node_id i, captype cap_source, captype cap_sink)
	{
	...
	/*
	    ...
	    to a forward star GraphCuts representation.
	    ...
	    arc block.)
	*/
	void GraphCuts::prepare_GraphCuts()
	{
	    node *i;
